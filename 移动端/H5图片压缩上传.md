...
// base64地址图片加载完毕后
img.onload = function () {	//第四步：监听图片加载完成
    // 图片原始尺寸
    ...
    ctx.drawImage(img, 0, 0, targetWidth, targetHeight); //第五步：将图片画入画布中
    //图片压缩 
    //toDataURL把canvas画布转为img图像（转换为base64格式）
    //quality值越小，所绘制的图片越模糊
    var data = canvas.toDataURL('image/jpeg',0.59); //第六步： 压缩图片
    //上传图片(使用formdata上传)
    ...
    //上传图片（使用ajaxfileupload.js插件上传）
};

// 文件base64化，以便获知图片原始尺寸
reader.onload = function(e) {
    img.src = e.target.result;   //第三步： 获取画布所需要的文件
};
eleFile.addEventListener('change', function (event) {	//第一步：监听change事件
    file = event.target.files[0];
    // 选择的文件是图片
    if (file.type.indexOf("image") == 0) {
        reader.readAsDataURL(file);    //第二步：把图片转为base64格式
    }
});
